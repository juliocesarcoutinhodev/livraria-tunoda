...
import { useBookDetailAdmin, useUpdateBook, useAuthors } from "@/hooks";
import { useAutoLogoutAfterInactivity } from "@/hooks/useInactivityLogout";
import { getErrorMessage } from "@/lib/api-client";
import type { UpdateBookRequest } from "@/types/book";
import type { Currency, WeightUnit, ResourceStatus } from "@/types/api";
...
export default function EditBookPage() {
  ...
  const { data: book, isLoading: isLoadingBook } = useBookDetailAdmin(bookId);
  ...
  // Preencher formulário quando livro carregar
  useEffect(() => {
    if (book) {
      setFormData({
        title: book.title,
      ...
      setDisplayValues({
        ...
        price: formatCurrency((book.price * 100).toString(), book.currency),
        weight: formatWeight((book.weight * 1000).toString()),
      });
    }
  }, [book]);
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.isbn && !isValidISBN(formData.isbn)) {
      newErrors.isbn = "ISBN inválido (deve ter 10 ou 13 dígitos)";
    }
