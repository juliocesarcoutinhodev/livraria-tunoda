...
export default function CheckoutPage() {
  ...
  const selectedOption = useMemo<ShippingOption | null>(() => {
    ...
    return (
      shippingQuote.options.find(
        (option) => option.serviceCode === selectedService
      ) || null
    );
  ...
  const fetchShippingQuote = async (postalCode: string) => {
    ...
    try {
      ...
      if (calculated.options.length > 0) {
        const firstOption = calculated.options[0];
        setSelectedService(firstOption.serviceCode);
        await shippingService.selectOption(quote.id, firstOption.serviceCode);
      }
    } catch {
                        ...
                        shippingQuote.options.map((option) => (
                          ...
                          >
                            <input
                              ...
                              checked={selectedService === option.serviceCode}
                              onChange={() =>
